...
export default function Materiais({ usuarioLogado }) {
  ...
  // Formulário Cadastro / Edição de Material
  ...
  const [estado, setEstado] = useState('Perfeito'); // 'Perfeito', 'Avariado', 'Em Manutenção'
  ...
  const uploadFotoAvaria = async (file) => {
    if (!file) return;
  ...
  const totalAvariados = materiais.filter(m => m.estado !== 'Perfeito').length;

  return (
    <div className="space-y-6">
      {/* Banner de Resumo da Guarita */}
                  ...
                  <div>
                    ...
                    <label className="w-full bg-red-700 hover:bg-red-800 text-white font-bold py-2.5 px-3 rounded-lg cursor-pointer flex items-center justify-center gap-2 text-xs transition">
                      ...
                      <input
                        ...
                        accept="image/*"
                        capture="environment"
                        onChange={(e) => uploadFotoAvaria(e.target.files[0])}
                        className="hidden"
