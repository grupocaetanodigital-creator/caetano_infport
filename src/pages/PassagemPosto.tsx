...
export default function PassagemPosto({ usuarioLogado, onTrocarOperador }: PassagemPostoProps) {
  ...
  const [resumoPendencias, setResumoPendencias] = useState({
    ...
    materiaisOk: true,
    qtdMateriais: 0,
    listaMateriaisAvariados: [] as any[],
    ocorrenciasAbertas: 0,
    listaOcorrencias: [] as any[],
  ...
  const varrerPendenciasModulos = async () => {
    ...
    try {
      ...
      const chavesFora = (chavesData || []).filter((c: any) => {
        const st = (c.status || '').trim().toLowerCase();
        return st !== 'disponivel' && st !== 'disponível' && st !== 'no quadro';
      });

      // 5. Inventário/Materiais do Posto com Avaria (tabela canônica: materiais_posto)
      const { data: materiaisData } = await supabase
        .from('materiais_posto')
        .select('*')
        .eq('condominio_id', condId);

      const materiaisAvariados = (materiaisData || []).filter((m: any) => 
        m.estado && m.estado !== 'Perfeito'
      );
      ...
      const ocorrenciasAbertas = (ocorrenciasData || []).filter((o: any) => {
