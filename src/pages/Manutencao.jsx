...
export default function Manutencao({ usuarioLogado }) {
  ...
  // Navegação Interna: 'chamados' ou 'checklist'
  const [subAba, setSubAba] = useState('checklist');

  // ESTADOS DO CHECKLIST / ROTINAS
  ...
  // Modal Nova Rotina / Item de Checklist
  ...
  // Modal Alterar Status / Concluir Task
  ...
  }, [subAba, filtroFrequencia, filtroStatusRotina, filtroStatusOS]);
  ...
  // Salvar Novo Item de Checklist
  const criarRotina = async (e) => {
    e.preventDefault();
  ...
  // Atualizar Status da Tarefa de Rotina
  const atualizarStatusRotina = async (e) => {
    e.preventDefault();
    setLoading(true);
  ...
  const rotinasFiltradas = rotinas.filter(item => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* CONTEÚDO SUB-ABA 1: CHECKLIST & FILA DO MANUTENCISTA */}
      {subAba === 'checklist' && (
      ...
      {/* CONTEÚDO SUB-ABA 2: CHAMADOS E OS PONTUAIS */}
      {subAba === 'chamados' && (
        <div className="space-y-4">
      ...
      {/* MODAL ADICIONAR ITEM NA ROTINA */}
      {modalNovaRotina && (
      ...
      {/* MODAL ATUALIZAR STATUS DA ROTINA */}
      {modalStatus && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl relative">
              ...
              <div className="space-y-1.5">
                ...
                <label className="w-full bg-slate-900 text-white font-bold py-3 px-4 rounded-xl cursor-pointer flex items-center justify-center gap-2 text-xs">
                  ...
                  <input
                    ...
                    accept="image/*"
                    capture="environment"
                    onChange={(e) => uploadFoto(e.target.files[0], 'checklist_fotos', setFotoRotinaUrl)}
                    className="hidden"
